...
-- common.LogInfo("","OnAlchemyStarted")
 local ttt = "LibreAlchemy: Приветствую!"
 cnt=cnt+1
...
-- common.LogInfo("","OnAlchemyReactionFinished")
 ...
 if ainf~=nil then
  ...
  if table.getn(lFound)==0 then
   local vt=common.CreateValuedText()
   local ttt = "Тут ничего, кроме бормотухи."
   wSetText(ttt,0)
   isHello=false
  else
  ...
  local ttt = "LibreAlchemy: ошибка какая-то."
  wSetText(ttt,0)
  isHello=false
 end 
end
...
-- dbg_out(isPlacing,"isPlacing")
 ...
 isCongr=false
 if rc>0 then
  ttt="Возможно, есть рецепты: "..tostring(rc).." шт."
  wSetText(ttt,1)
  isHello=false
